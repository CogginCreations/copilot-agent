...
const riskReviewSchema = z.object({
    ...
    finalWatchlist: z.array(z.object({
        ticker: z.string(),
        weightGuidance: z.string().optional(),
        rationale: z.string(),
    })),
});
export const riskManagerAgent = {
    name: "risk-manager",
    model: "gpt-4.1-mini",
    displayName: "Risk Manager",
    description: "Reviews risk, concentration, and drawdown exposure",
    prompt: "Review outputs for risk and improve downside profile. Return JSON with topRisks, adjustments, and finalWatchlist.",
